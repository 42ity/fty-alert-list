...
static int convert_file(const char* file_name, const char* old_path, const char* new_path)
{
    assert(file_name);
    assert(old_path);
    assert(new_path);
    ...
    while (offset < cursize) {
        ...
        log_debug("prefix == %" PRIu64 "; offset = %jd ", uint64_t(*prefix), offset);
        ...
        zchunk_extend(nchunk, reinterpret_cast<const void*>(&size), sizeof(uint64_t));
        // data
        zchunk_extend(nchunk, reinterpret_cast<const void*>(zframe_data(nframe)), size);
