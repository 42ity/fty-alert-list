...
struct _bios_proto_t
{
    ...
    uint32_t  ttl;  //  Metric time to live seconds (i.e. How long is the metric valid - At the latest how long from now
    ...
    switch (self->id) {
        case BIOS_PROTO_METRIC:
            ...
            if (self->aux) {
                ...
                char* item = reinterpret_cast<char*>(zhash_first(self->aux));
                while (item) {
                    self->aux_bytes += 1 + strlen(zhash_cursor(self->aux));
                    self->aux_bytes += 4 + strlen(item);
                    item = reinterpret_cast<char*>(zhash_next(self->aux));
                }
            }
        ...
        case BIOS_PROTO_ASSET:
            ...
            if (self->ext) {
                self->ext_bytes = 0;
                //  Add up size of dictionary contents
                char* item = reinterpret_cast<char*>(zhash_first(self->ext));
                while (item) {
                    self->ext_bytes += 1 + strlen(zhash_cursor(self->ext));
                    self->ext_bytes += 4 + strlen(item);
                    item = reinterpret_cast<char*>(zhash_next(self->ext));
                }
            }
...
bios_proto_t* bios_proto_dup(bios_proto_t* self)
{
    if (!self)
        return NULL;
...
uint64_t bios_proto_aux_number(bios_proto_t* self, const char* key, uint64_t default_value)
{
    ...
    char*    string = NULL;
    if (self->aux)
        string = reinterpret_cast<char*>(zhash_lookup(self->aux, key));
    if (string)
...
uint64_t bios_proto_ext_number(bios_proto_t* self, const char* key, uint64_t default_value)
{
    ...
    char*    string = NULL;
    if (self->ext)
        string = reinterpret_cast<char*>(zhash_lookup(self->ext, key));
    if (string)
        value = uint64_t(atol(string));
...
void bios_proto_test(bool verbose)
{
    printf(" * bios_proto: ");
